                    ...
                    <div class="flex items-center justify-end mt-4">
                        <form action="{{ route('despacho.cambiarStatus', $despachoActivo->id) }}" method="POST">
                            ...
                            <button type="submit" class="btn btn-success"
                                onclick="return confirm('¿Estás seguro de que deseas Descontar del Inventario?')">
                                {{ __('Cargar Despacho') }}
                            </button>
                        </form>
                    </div>
                                        ...
                                        <div class="form-floating mb-3">
                                            <select id="productos_id" name="productos_id" class="form-select">
                                                ...
                                                <option value="{{ $producto->id }}">{{ $producto->nombre }} - {{
                                                    $producto->codigo }} - {{ $producto->categoria->name }} - {{
                                                    $producto->stock_actual }}</option>
                                                @endforeach
                                            </select>
                    ...
                    <table class="table table-hover">
                        ...
                        <tbody>
                            ...
                            <tr>
                                ...
                                <td>{{ number_format(($detalle->precio_dolar*$detalle->cantidad),
                                    2)*$despachoActivo->tasa->tasa }}</td>
                                <td>
                                    <button type="button" class="btn btn-primary btn-sm" data-bs-toggle="modal"
                                        data-bs-target="#EditarProducto{{ $detalle->id }}">Editar</button>
                                    <!-- Modal para editar producto -->
                                    <div class="modal fade" id="EditarProducto{{ $detalle->id }}" tabindex="-1"
                                        aria-hidden="true">
                                        <div class="modal-dialog">
                                            <div class="modal-content">
                                                <div class="modal-header">
                                                ...
                                                <form
                                                    action="{{ route('detalleDespacho.updateDetalle', $detalle->id) }}"
                                                    method="POST">
                                                    @csrf
                                                    @method('PUT')
                                                    <div class="modal-body">
                                                        <div class="form-floating mb-3">
                                                            ...
                                                            <x-input-error :messages="$errors->get('producto_id')"
                                                                class="mt-2" />
                                                        </div>
                                                        <div class="form-floating mb-3">
                                    ...
                                    <button type="button" class="btn btn-danger btn-sm" data-bs-toggle="modal"
                                        data-bs-target="#EliminarProducto{{ $detalle->id }}">Eliminar</button>
                                    <!-- Modal para eliminar producto -->
                                    <div class="modal fade" id="EliminarProducto{{ $detalle->id }}" tabindex="-1"
                                        aria-hidden="true">
                                        <div class="modal-dialog">
                                            <div class="modal-content">
                                                <div class="modal-header">
                                                ...
                                                <form
                                                    action="{{ route('detalleDespacho.destroyDetalle', $detalle->id) }}"
                                                    method="POST">
                                                    @csrf
                                                    @method('DELETE')
                                                    <div class="modal-body">
